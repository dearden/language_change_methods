...
def split_members(member_list, percentage=0.5, random_state=None):
    """
    Randomly splits group members into Snapshot and Test samples based on the percentage.
    :param mp_list: Numpy array of memberss.
    :percentage: Percentage of members to make Snapshot samples (e.g. 0.6 -> 60% snapshot, 40% test.)
    """
    if random_state is not None:
        random_state.shuffle(member_list)
    else:
        np.random.shuffle(member_list)

    snap_members = member_list[:int((len(member_list)+1)*percentage)]
    test_members = member_list[int((len(member_list)+1)*percentage):]

    return snap_members, test_members


def get_snap_and_test_no_limit(all_contribs, contributions, reference_contribs, win_func, win_size, win_step, percentage_split=0.6, random_state=None, member_field="member"):
    """
    Splits the given contributions into snapshot and testing samples.
    """
    # Get members for each group
    members = contributions[member_field].unique()

    # Split members into snapshot and train
    members_snap, members_test = split_members(members, percentage_split, random_state=random_state)

    # Get the snap and test samples
    snap = contributions[contributions[member_field].isin(members_snap)]
    test = contributions[contributions[member_field].isin(members_test)]

    if reference_contribs is not None:
        # Get the snap and test samples for the reference
        ref_snap = reference_contribs[reference_contribs[member_field].isin(members_snap)]
        ref_test = reference_contribs[reference_contribs[member_field].isin(members_test)]
    else:
...
def get_snap_and_test_no_limit_bal(all_contribs, gnames, group_contribs, reference_contribs, win_func, win_size, win_step, percentage_split=0.6, random_state=None, member_field="member"):
    """
    ...
    for i, g in enumerate(gnames):
        # Get members for each group
        members = group_contribs[i][member_field].unique()

        # Split members into snapshot and train
        members_snap[g], members_test[g] = split_members(members, percentage_split, random_state=random_state)

    # Get the minimum number of members for snap and test
    min_snap = min([len(members_snap[g]) for g in members_snap])
    min_test = min([len(members_test[g]) for g in members_test])

    # Balance the number of members
    members_snap = {g: members_snap[g][:min_snap] for g in gnames}
    members_test = {g: members_test[g][:min_test] for g in gnames}
    ...
    for i, g in enumerate(gnames):
        # Get the snap and test samples
        snap[g] = group_contribs[i][group_contribs[i][member_field].isin(members_snap[g])]
        test[g] = group_contribs[i][group_contribs[i][member_field].isin(members_test[g])]

        if reference_contribs is not None:
            # Get the snap and test samples for the reference
            ref_snap[g] = reference_contribs[reference_contribs[member_field].isin(members_snap[g])]
            ref_test[g] = reference_contribs[reference_contribs[member_field].isin(members_test[g])]
        else:
            ref_snap, ref_test = None, None
...
def multi_member_splits(gnames, group_contributions, toks, reference, ref_toks, window_func, window_size, window_step,
                        n_runs=5, comp_method="CE", n_words_per_contribution=60, balanced_groups=False, random_state=None, member_field="member"):
    """
    Over n runs, splits members into Test and Snapshot samples and calculates the cross-entropy of the test samples according to the snapshot samples for each group.
    """
    comparison_methods = {
    ...
    for run_num in range(n_runs):
        ...
        # Num users
        curr_num_snap_users = {g: {w: len(snap_samples[g][w][member_field].unique()) for w in snap_samples[g].index} for g in gnames_w_ref}
        curr_num_test_users = {g: {w: len(test_samples[g][w][member_field].unique()) for w in test_samples[g].index} for g in gnames_w_ref}
...
def sample_n_contribs_per_MP(all_contribs, group_contributions, n_contribs_per_mp, window_func, window_size, window_step, random_state=None, member_field="member"):
    """
    ...
    for window, window_contribs in window_func(all_contribs, window_size, window_step):
        ...
        for MP, ids in curr_contribs.groupby(member_field).groups.items():
...
def get_samples_by_these_members(all_contribs, contribs, members_snap, members_test, contrib_limit, win_func, win_size, win_step, random_state=None, member_field="member"):
    """
    Gets the samples for the given snapshot and test members.
    """

    # Get the Labour and Conservative split samples
    snap = contribs[contribs[member_field].isin(members_snap)]
    test = contribs[contribs[member_field].isin(members_test)]

    # Get sample for labour and conservative
    snap_sampled = sample_n_contribs_per_MP(all_contribs, snap, contrib_limit, win_func, win_size, win_step, member_field=member_field, random_state=random_state)
    test_sampled = sample_n_contribs_per_MP(all_contribs, test, contrib_limit, win_func, win_size, win_step, member_field=member_field, random_state=random_state)

    return snap_sampled, test_sampled


def get_snap_and_test_with_limit(all_contribs, group_contribs, reference_contribs, contrib_limit, win_func, win_size, win_step, percentage_split=0.6, random_state=None, member_field="member"):
    """
    Splits the given contributions into snapshot and test samples, with a limit of contributions per mp.
    """
    # Get members for each group
    members = group_contribs[member_field].unique()

    # Split members into snapshot and train
    members_snap, members_test = split_members(members, percentage_split, random_state=random_state)

    # Get the snap and test samples
    snap, test = get_samples_by_these_members(all_contribs, group_contribs, members_snap, members_test, contrib_limit, win_func, win_size, win_step, member_field=member_field, random_state=random_state)

    if reference_contribs is not None:
        # Get the snap and test samples for the reference
        ref_snap, ref_test = get_samples_by_these_members(all_contribs, reference_contribs, members_snap, members_test, contrib_limit, win_func, win_size, win_step, member_field=member_field, random_state=random_state)
    else:
        ref_snap, ref_test = None, None
...
def get_snap_and_test_with_limit_bal(all_contribs, gnames, group_contribs, reference_contribs, contrib_limit, win_func, win_size, win_step, percentage_split=0.6, random_state=None, member_field="member"):
    """
    Splits the given contributions into snapshot and test samples, with a limit of contributions per mp.

    Also ensures that all groups have the same number of members in Snap and Test samples.
    """
    members_snap = dict()
    members_test = dict()
    ...
    for i, g in enumerate(gnames):
        # Get the snap and test samples
        snap[g], test[g] = get_samples_by_these_members(all_contribs, group_contribs[i], members_snap[g], members_test[g], contrib_limit, win_func, win_size, win_step, member_field=member_field, random_state=random_state)

        if reference_contribs is not None:
            # Get the snap and test samples for the reference
            ref_snap[g], ref_test[g] = get_samples_by_these_members(all_contribs, reference_contribs, members_snap[g], members_test[g], contrib_limit, win_func, win_size, win_step, member_field=member_field, random_state=random_state)
        else:
            ref_snap, ref_test = None, None
...
def multi_member_splits_with_limit(gnames, group_contributions, toks, reference, ref_toks,
                               window_func, window_size, window_step,
                               n_contribs_per_mp=5, n_runs=5,
                               comp_method="CE", n_words_per_contribution=60, balanced_groups=False, random_state=None, member_field="member"):
    """
    Over n runs, splits members into Test and Snapshot samples and calculates the cross-entropy of the test samples according to the snapshot samples for each group.
    ...
    for run_num in range(n_runs):
        ...
        if balanced_groups:
            # Gets snapshot and test samples with same number of members for each group
            snap_samples, test_samples, \
...
def plot_group_similarity_across_runs_stepped(means, stds, ax=None, colour="black", label="", line_style="-", line_alpha=1, line_width=3, point_alpha=0.75, fill_alpha=0.4, key_dates=None, end_dates=None):
    """
    ...
    ax.plot(n_means, drawstyle='steps-post', linestyle=line_style, color=colour, label=label, alpha=line_alpha, lw=line_width)
...
def get_end_of_windows(contributions, window_func, window_size, window_step, time_column="time"):
    end_dates = []
    for window, contribs in window_func(contributions, window_size, window_step):
        end_dates.append(contribs.sort_values(time_column, ascending=True).iloc[-1][time_column])
